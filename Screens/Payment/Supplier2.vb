...
Public Class Supplier2
    ...
    Public a1, b1, c1, d1, e1 As String
    Dim a2b As Integer
    Private Sub Supplier2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        BindGD()
    End Sub
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        con.Open()
        Dim cmd As New SqlCommand("Select count(*) from Supplier2 where PurID = @PurID", con)
        ...
        If count > 0 Then
            ...
            Dim sql As String = "Select PayID,PurID,SID,PID,PayType,Amt,PayDate,PayTime from Supplier2 where PurID ='" & Convert.ToInt32(TextBox9.Text) & "'"
            Using connection As New SqlConnection("Data Source=THEDS482\SQLEXPRESS;Initial Catalog=SIS482;Integrated Security=True")
                ...
                Using command As New SqlCommand(sql, connection)
                    Using adapter As New SqlDataAdapter(command)
                        Dim dt As New DataTable("Supplier2")
                        adapter.Fill(dt)
            ...
            End Using
        Else
            MsgBox("Payment ID Not Found", MsgBoxStyle.Information)
        End If
        con.Close()
    End Sub
    Public Sub BindGD()
        Dim ConnectionString As String = "Data Source=THEDS482\SQLEXPRESS;Initial Catalog=SIS482;Integrated Security=True"
        Dim sql As String = "Select PayID,PurID,SID,PID,PayType,Amt,PayDate,PayTime from Supplier2 order by ID asc"
        Using connection As New SqlConnection("Data Source=THEDS482\SQLEXPRESS;Initial Catalog=SIS482;Integrated Security=True")
            connection.Open()
            Using command As New SqlCommand(sql, connection)
                Using adapter As New SqlDataAdapter(command)
