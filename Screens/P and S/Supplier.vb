...
Public Class Supplier
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        If (IsFormValid()) Then
            qry = "Insert into Supplier values ('" & TextBox1.Text & "','" & TextBox2.Text & "','" & TextBox3.Text & "','" & TextBox4.Text & "','" & TextBox5.Text & "')"
            Dim logincorrect As Boolean = Convert.ToBoolean(InsertData(qry))
            If (logincorrect) Then
                MsgBox("Supplier Added Sucessfully", MsgBoxStyle.Information)
                Clr()
            Else
    ...
    Private Function IsFormValid() As Boolean
        ...
        ElseIf (TextBox3.Text.Trim() = String.Empty) Then
            MsgBox("Supplier Address is Required", MsgBoxStyle.Critical)
            Clr()
            Return False
        ElseIf (TextBox4.Text.Trim() = String.Empty) Then
            MsgBox("Supplier PhoneNo is Required", MsgBoxStyle.Critical)
            Clr()
            Return False
        ElseIf (TextBox5.Text.Trim() = String.Empty) Then
            MsgBox("Supplier EmailID is Required", MsgBoxStyle.Critical)
            Clr()
            Return False
        End If
        Return True
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        If (IsFormValid()) Then
            qry = "Update Supplier set SID ='" & TextBox1.Text & "',SName ='" & TextBox2.Text & "' ,SAddress ='" & TextBox3.Text & "',SPhoneNo ='" & TextBox4.Text & "' ,SEmailID ='" & TextBox5.Text & "'where SID ='" & Convert.ToInt32(TextBox1.Text) & "'"
            Dim isupdatetrue As Boolean = Convert.ToBoolean(InsertData(qry))
            If (isupdatetrue) Then
                MsgBox("Supplier Updated Sucessfully", MsgBoxStyle.Information)
                btnAdd.Enabled = True
                Clr()
            Else
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If result = DialogResult.Yes Then
            If (IsFormValid2()) Then
                qry = "Delete from Supplier where SID ='" & Convert.ToInt32(TextBox1.Text) & "'"
                Dim wantToDelete As Boolean = Convert.ToBoolean(InsertData(qry))
                If (wantToDelete) Then
                    MsgBox("Supplier Deleted Sucessfully", MsgBoxStyle.Information)
                    btnAdd.Enabled = True
    ...
    Private Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        cmd.Parameters.AddWithValue("@SID", TextBox9.Text)
        Dim count As Integer = Convert.ToInt32(cmd.ExecuteScalar())
        If count > 0 Then
            Dim ConnectionString As String = "Data Source=THEDS482\SQLEXPRESS;Initial Catalog=SIS482;Integrated Security=True"
            Dim sql As String = "Select SID,SName,SAddress,SPhoneNo,SEmailID from Supplier where SID ='" & Convert.ToInt32(TextBox9.Text) & "'"
    ...
    Public Sub BindGD()
        Dim ConnectionString As String = "Data Source=THEDS482\SQLEXPRESS;Initial Catalog=SIS482;Integrated Security=True"
        Dim sql As String = "Select SID,SName,SAddress,SPhoneNo,SEmailID from Supplier order by ID asc"
        Using connection As New SqlConnection("Data Source=THEDS482\SQLEXPRESS;Initial Catalog=SIS482;Integrated Security=True")
            connection.Open()
            Using command As New SqlCommand(sql, connection)
                Using adapter As New SqlDataAdapter(command)
    ...
    Private Sub TextBox5_Leave(sender As Object, e As EventArgs) Handles TextBox5.Leave
        If Not IsValidEmail(TextBox5.Text) Then
            MsgBox("Invalid EmailID", MsgBoxStyle.Information)
        End If
    End Sub
        ...
        If r1.Length >= 2 Then
            ...
            If Integer.TryParse(30, Nothing) Then
                ...
                If number = 30 Then
                    ...
                    Dim count As Integer = Convert.ToInt32(cmd.ExecuteScalar())
                    If count > 0 Then
                        MsgBox("Supplier ID already Exist", vbOKOnly + MsgBoxStyle.Information)
                        TextBox1.Text = "30"
                    End If
                    Return True
                    con.Close()
